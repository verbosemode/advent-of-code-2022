...
module Part_1 = struct
  ...
  let items_of_line s =
    try (item_of_char_exn (String.get s 0), item_of_char_exn (String.get s 2))
    with (Invalid_argument _ | Parser_error) as e -> raise e

  let calculate_my_score (other, ours) =
    match (other, ours) with
    | Rock, Rock | Paper, Paper | Scissors, Scissors ->
        draw + item_to_score ours
...
module Part_2 = struct
  ...
  let items_of_line s =
    try
      ( Part_1.item_of_char_exn (String.get s 0),
        item_of_char_exn (String.get s 2) )
    with (Invalid_argument _ | Parser_error) as e -> raise e
